...
function ChangePassword() {
  ...
  async function onFormSubmit(e) {
    e.preventDefault();
    if (!userInput.oldPassword || !userInput.newPassword || !userInput.confirmPassword) {
      toast.error("All field are required");
      return;
    }
    if(userInput.newPassword !== userInput.confirmPassword){
        toast.error("Confirm doesn't match");
        return
    }
    try {
      ...
      toast.promise(response, {
        ...
        success: "Your password change successfuly",
        error: "Password change failed, Please try again later",
      });
      const finalResponse = await (response);
      console.log(finalResponse);
      if(finalResponse?.data?.validPassword === false){
        toast.error("Enter old password is wrong!")
        return
      }
